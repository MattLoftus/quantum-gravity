...
For 4-orders at $N = 30$, the correlation is $r = 0.91 \pm 0.03$---slightly stronger than the 2D result ($r = 0.88$). The ER=EPR relationship persists and strengthens in the physically relevant dimension. We note that Theorem~\ref{thm:cross} is proved only for 2-orders; for $d$-orders with $d > 2$, the cross terms $X_{ij}$ need not vanish identically (though they may remain small). The strong 4D correlation suggests that the analytic structure generalizes, but a formal proof for $d$-orders remains open.
...
Define the \emph{causal signature vector} of element $i$ as $S_{ik} = C_{ik} - C_{ki}$, where $C$ is the causal order matrix. Thus $S_{ik} = +1$ if $i \prec k$, $S_{ik} = -1$ if $k \prec i$, and $S_{ik} = 0$ if $i$ and $k$ are spacelike or $i = k$. The Pauli-Jordan function is $\Delta_{ij} = (2/N) S_{ji}$, and $(-\Delta^2)_{ij} = (4/N^2) \sum_k S_{ik} S_{jk}$.
...
\label{thm:gram}
For spacelike pairs $(i,j)$ in a 2-order,
\begin{equation}
...
\begin{proof}
Follows directly from $(-\Delta^2)_{ij} = (4/N^2) \sum_k S_{ik} S_{jk}$ and Theorem~\ref{thm:cross}.
\end{proof}
