...
The 2-order Hasse diagram has algebraic connectivity $\sim$6--79$\times$ larger than density-matched random DAGs (Table~\ref{tab:fiedler}). The ratio grows with $N$ because $\lambda_2$ increases for 2-orders but decreases for random DAGs. This means Minkowski-embedded causal sets have high algebraic connectivity in their Hasse diagrams, in contrast to random DAGs whose Hasse diagrams fragment into near-trees.

A power-law fit to the Fiedler value gives $\lambda_2 \sim N^{0.318 \pm 0.012}$ with $R^2 = 0.993$, confirming systematic growth with system size. We note, however, that this does \emph{not} establish an expander family in the strict sense: the maximum degree $d_{\max}$ grows as $\sim\!N^{0.45}$, faster than $\lambda_2 \sim N^{0.32}$, so the Cheeger ratio $\lambda_2 / d_{\max} \to 0$ as $N \to \infty$. The Hasse diagrams have high algebraic connectivity but are not expanders.
...
\textbf{2. High connectivity and locality.} The high Fiedler value of Minkowski-embedded causets ($\lambda_2 \sim N^{0.32}$) means the Hasse diagram is well-connected, though not a true expander family (since $d_{\max} \sim N^{0.45}$ grows faster). Physically, the high algebraic connectivity reflects the locality of Minkowski spacetime: every spacetime region is connected to every other by short chains of links, with no bottlenecks. Random DAGs lack this spatial locality, producing fragmented Hasse diagrams with low connectivity. The triangle-free property (girth $\geq 4$) is a direct consequence of transitivity in the partial order.
...
The Hasse diagram of causal sets carries rich geometric information that complements transitive-closure-based observables. The algebraic connectivity is $\sim$50$\times$ larger for Minkowski-embedded causets than for random DAGs and scales as $\lambda_2 \sim N^{0.318 \pm 0.012}$ ($R^2 = 0.993$), though $d_{\max} \sim N^{0.45}$ grows faster, so the Hasse diagrams are not a true expander family. The Fiedler value jumps by 74\% at the BD phase transition and encodes the embedding dimension via monotonic decrease with $d$. The Hasse diagram is triangle-free (girth $\geq 4$) with independence number $\alpha \sim N^{0.83}$. The link fraction provides a perfectly monotonic order parameter for the BD transition, capturing the restructuring of irreducible causal relations. These results establish spectral graph theory on the Hasse diagram as a new tool for causal set quantum gravity.
...
Class.\ Quant.\ Grav.\ \textbf{35}, 065003 (2018),
