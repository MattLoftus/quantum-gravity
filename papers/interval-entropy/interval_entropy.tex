...
A \emph{2-order} on $N$ elements is defined by two permutations $U$ and $V$ of $\{1, \ldots, N\}$. Element $i$ precedes element $j$ if and only if $u_i < u_j$ and $v_i < v_j$. Every 2-order faithfully embeds into 2-dimensional Minkowski spacetime~\cite{rideout2000}, making this the natural sample space for 2D causal set quantum gravity.
...
The spectral dimension $d_s$, defined via the return probability of a random walk on the link graph, has been widely used to probe emergent geometry in quantum gravity~\cite{carlip2017}. We find that $d_s$ is \emph{not} a useful observable for distinguishing the BD phases.
...
This establishes that link-graph spectral dimension on causal sets is purely a function of connectivity and does not probe causal geometry. Interval entropy, chain dimension, and layer structure are superior probes of the phase transition.
...
\begin{equation}
S_{4\mathrm{D}} = \frac{1}{24}\left( N - 4L + 6\mathcal{N}_1 - 4\mathcal{N}_2 \right),
\end{equation}
...
This non-monotonic behavior---absent in 2D---suggests that the 4D BD action admits at least two ordered phases with different interval structure. The recovery of $H$ in the deep ordered phase is particularly striking: the most strongly ordered configurations ($\beta \gg \beta_c$) have \emph{more diverse} interval distributions than the intermediate ones.
