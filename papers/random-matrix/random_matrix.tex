...
The GUE statistics are not specific to causal sets. Table~\ref{tab:null} shows that \emph{any} sufficiently dense random antisymmetric matrix produces $\langle r \rangle \approx 0.60$, regardless of whether entries are $\pm 1$, sparse, or Gaussian. This follows from Erd\H{o}s-Yau-type universality~\cite{erdos2012}: the Pauli-Jordan entries are sufficiently uncorrelated (mean $|\mathrm{corr}| = 0.035$ for non-overlapping pairs) and dense ($\sim$50\% nonzero, well above the empirical GUE threshold of $\sim$5\%) for local universality to apply.
...
An earlier analysis of the same system reported $\langle r \rangle \approx 0.12$ near the BD transition, suggesting anomalous eigenvalue clustering. We now identify this as a \emph{phase-mixing artifact}. At $\beta \approx \beta_c$, the MCMC sampler oscillates between continuum-like and crystalline-like configurations. If eigenvalue spectra from both phases are concatenated into a single list (as happens when averaging over MCMC samples that span both phases), the interleaving of two distinct spectral sequences produces spurious small spacings.

\textbf{Direct test.} We independently generate continuum ($\beta = 0$) and pure KR spectra, each with $\langle r \rangle \approx 0.56$. Concatenating the two eigenvalue lists and recomputing gives $\langle r \rangle \approx 0.42$---dramatically suppressed from GUE ($0.60$) and close to Poisson ($0.39$), confirming the mechanism: phase mixing during MCMC creates artificial spectral suppression. More severe mixing (e.g., interleaving spectra with greater spectral mismatch) can push $\langle r \rangle$ further below Poisson, explaining the originally reported sub-Poisson values.
...
While the level spacing ratio (a nearest-neighbor statistic) matches GUE, the number variance $\Sigma^2(L)$ grows linearly with $L$ rather than logarithmically ($\Sigma^2 \sim (1/\pi^2)\ln L$ for GUE). This indicates that GUE universality is \emph{short-range} only: nearest-neighbor level repulsion follows GUE, but long-range spectral correlations are weaker than in a true GUE ensemble. The eigenvalue density is heavy-tailed (kurtosis $\approx 53$, far from the semicircle kurtosis of $-1$), confirming that the global spectral structure differs from GUE while local statistics are universal.
